...
class SaleOrder(models.Model):
    ...
    def descriptive_link(self):
            ...
            for line in altered_products:
                msg_line = (
                    ...
                    % (
                        ...
                        line.qty_done,
                        line.qty_done * line.product_id.standard_price,
                    )
                )
        ...
        if missing_product_lines:
            missing_message = (
                "\nOs seguintes itens não estão mais disponíveis em estoque:\n\n"
            )
        ...
        replacement_message = ""
        replacement_product_lines = picking_id.move_ids_without_package.filtered(
            lambda move: move.sale_line_id.is_replacement == 1
    ...
    def request_customer_approval(self):
        ...
        if len(products) > 1:
            text_str = (
                "Olá %s! Infelizmente os produtos %s não estão "
                "disponíveis para o seu pedido %s. "
                % (self.partner_invoice_id.name, product_names, self.name)
            )
        else:
            text_str = (
                "Olá %s! Infelizmente o produto %s não está "
    ...
    def get_donation_price(self, percentage=0.35):
        valid_lines = self.order_line.filtered(
            lambda l: l.product_id.default_code != "DON"
        )
        order_subtotal = sum(
            line.price_unit * line.move_ids.move_line_ids.qty_done
    ...
    def recompute_freight_distribution(self):
        ...
        valid_lines = self.order_line.filtered(lambda l: l.product_id.donation != True)
        ...
        for line in valid_lines:
            if line != valid_lines[-1]:
                line_freight = (
                    amount_freight * line.qty_delivered * line.price_unit / order_total
                )
    ...
    def action_confirm(self):
        ...
        if not self.carrier_id:
            raise ValidationError(
                "Você não pode confirmar uma Ordem de Venda sem antes escolher um"
                " método de entrega."
            )
...
class Picking(models.Model):
    ...
    def action_done(self):
        ...
        for record in self.filtered(lambda ot: ot.picking_type_id.code == "outgoing"):
