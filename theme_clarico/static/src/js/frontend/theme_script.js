...
odoo.define('theme_emipro.theme_script', function (require) {
    ...
    function quickview() {
        $("a.quick-view-a").click(function () {
            ...
            ajax.jsonRpc('/productdata', 'call', { 'product_id': pid }).then(function (data) {
                // var secondary_unit = new sAnimation.registry.sale_secondary_unit();
                // secondary_unit.init();
    ...
    sAnimations.registry.themeWishlist = sAnimations.Class.extend({
        ...
        read_events: {
            'click .o_wish_rm_ept': '_onRemoveClick',
        },
    ...
    // sAnimation.registry.sale_secondary_unit.include({
    ...
    sAnimations.registry.themeEvent = sAnimations.Class.extend({
        ...
        _onMouseEnter: function (ev) {
            ...
            if ($(window).width() > 1200) {
                ...
                /*change by jayesh for shop page remove opacity from product image on hover*/
    ...
    sAnimations.registry.WebsiteSale.include({
        read_events: {
            ...
            'click #secondary_uom': 'onClickAddCartJSON',
            'change #secondary_uom': 'onClickAddCartJSON',
            'focusout .secondary-quantity': 'onClickAddCartJSON',
       ...
       */
        _onChangeCombination: function (ev, $parent, combination) {
            ...
            if (($uom_select) && ($uom_select.selectedIndex) && ($uom_select.selectedIndex != -1)) {
                uom_factor = $uom_select.options[$uom_select.selectedIndex].getAttribute('data-secondary-uom-factor');
            }

            var self = this;
            $price.html(self._priceToStr(combination.price));
            $default_price.html(self._priceToStr(combination.list_price));
            if ($parent.find(".oe_price .oe_currency_value")[1]) {
                $parent.find(".oe_price .oe_currency_value")[1].innerText = self._priceToStr(combination.price * parseInt($qty.value) * parseFloat(uom_factor.replace(",", ".")));
                var complete_price = self._priceToStr(combination.price * parseInt($qty.value) * parseFloat(uom_factor.replace(",", ".")));
                var contribution_price = self._priceToStr(combination.price * parseInt($qty.value) * 0.35 * parseFloat(uom_factor.replace(",", ".")));
                var description_complete_price = self._priceToStr(combination.price / 1.35);
        ...
        onClickAddCartJSON: function (ev) {
            ...
            var $price = document.querySelector(".oe_price .oe_currency_value");
            var $default_price = document.querySelector(".oe_default_price");
            var $optional_price = document.querySelector(".oe_optional");
            var $qty = document.querySelector('.secondary-quantity');
            if (!$qty) {
                $qty = document.querySelector('.quantity');
            }
            var $subtotal_price = document.querySelector(".oe_price")[1];
            var $uom_select = document.querySelector('#secondary_uom');
            var uom_factor = '1,0';
            if (($uom_select) && ($uom_select.selectedIndex) && ($uom_select.selectedIndex != -1)) {
                uom_factor = $uom_select.options[$uom_select.selectedIndex].getAttribute('data-secondary-uom-factor');
            ...
            var self = this;
            if ($('.oe_price .oe_currency_value')[1]) {
                var complete_price = self._priceToStr(parseFloat(($price.innerHTML).replace(",", ".")) * parseInt($qty.value) * parseFloat(uom_factor.replace(",", ".")));
                var contribution_price = self._priceToStr(parseFloat(($price.innerHTML).replace(",", ".")) * 0.35 * parseInt($qty.value) * parseFloat(uom_factor.replace(",", ".")));
                var description_complete_price = self._priceToStr(parseFloat(($price.innerHTML).replace(",", ".")) / 1.35);
    ...
    $(document).ready(function () {
        ...
        $(".o_portal_my_doc_table tbody tr").click(function () {
