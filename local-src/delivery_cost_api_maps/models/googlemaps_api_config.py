...
class GooglemapsApiConfig(models.Model):
    ...
    default_delivery_method_id = fields.Many2one(
        comodel_name="delivery.carrier", string="Default delivery carrier"
    )
    ...
    def get_distance_cost(self, origin, destination):
        if not origin or not destination:
            raise UserError(_("Origin or destination not provided!"))
        for record in self:
            try:
                client = googlemaps.Client(record.api_key)
                matrix = client.distance_matrix(origin, destination) or False
                distance = self.get_min_distance(matrix) if matrix else 0
    ...
    def button_teste(self):
        print(self.get_distance_cost("itajuba", "shopping vale sul sjc"))
...
class GooglemapsApiDistanceLines(models.Model):
    ...
    googlemaps_api_id = fields.Many2one(
        comodel_name="googlemaps.api.config", string="Googlemaps API"
    )
